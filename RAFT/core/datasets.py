...
class FlowDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False):
        ...
        if aug_params is not None:
            if sparse:
                self.augmentor = SparseFlowAugmenter(**aug_params)
            else:
                self.augmentor = FlowAugmenter(**aug_params)
...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='/datagrid/public_datasets/Sintel-complete', dstype='clean'):
...
class FlyingThings3D(FlowDataset):
    def __init__(self, aug_params=None, root='/datagrid/public_datasets/FlyingThings3D', dstype='frames_cleanpass'):
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.dashcam_augmenentation:
        aug_params = {'do_jpeg_transform': True,
                      'do_blend_transform': True,
                      'blend_source': args.blend_source,
                      'do_add_text_transform': True,
                      'blend_prop': 0.5,
                      'jpeg_prop': 0.5,
                      'add_text_prop': 0.5,
                      }
